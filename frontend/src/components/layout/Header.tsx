        ...
        {mounted ? (
          <>
            ...
            {user?.is_superadmin && (
              <Button
                variant="ghost"
                className="h-9 px-3 text-zinc-400 hover:text-zinc-900 hover:bg-zinc-50 rounded-xl transition-all text-xs font-bold gap-1.5"
              ...
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-2.5 p-1 pr-2 rounded-2xl hover:bg-zinc-50 transition-all group">
                  ...
                  <div className="hidden sm:flex flex-col items-start">
                    ...
                    {user?.is_superadmin && <span className="text-[9px] text-amber-600 font-bold uppercase tracking-widest mt-1">超级管理员</span>}
                    {!user?.is_superadmin && <span className="text-[9px] text-zinc-400 font-bold tracking-widest mt-1">租户用户</span>}
                  </div>
                </button>
              </DropdownMenuTrigger>
