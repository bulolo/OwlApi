...
export function GatewayDeployPanel({ gateway, onClose }: GatewayDeployPanelProps) {
  ...
  return (
    <div className="bg-white border border-border-subtle rounded-lg shadow-card p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold text-foreground">部署信息 — {gateway.name}</h3>
        <Button variant="ghost" onClick={onClose} className="h-8 px-3 text-xs font-bold text-muted-foreground">关闭</Button>
      </div>
