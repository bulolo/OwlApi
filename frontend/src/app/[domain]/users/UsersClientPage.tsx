  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900 tracking-tight">用户中心</h1>
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className={cn("flex flex-col space-y-3", selectedUserId ? "col-span-12 lg:col-span-7" : "col-span-12")}>
          <div className="bg-white/80 backdrop-blur-md border border-zinc-200/60 p-3 rounded-2xl flex gap-2 shadow-sm transition-all focus-within:shadow-md focus-within:border-blue-500/30">
            <div className="relative flex-1">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-400" />
              <Input
                placeholder="快速搜索姓名、邮箱或 ID..."
                className="pl-9 h-9 text-xs bg-zinc-50/50 border-zinc-100 rounded-xl focus:ring-1 transition-all"
                value={search}
                onChange={(e) => setSearch(e.target.value)}
              />
            </div>
            <Button variant="ghost" className="h-9 text-[10px] font-bold uppercase tracking-wide px-4 border border-zinc-100 hover:bg-zinc-50 text-zinc-500 rounded-xl">
          ...
          <div className="space-y-3">
            {filteredUsers.map((user) => (
              <motion.div
                ...
                className={cn(
                  "relative group cursor-pointer bg-white border rounded-2xl p-4 transition-all duration-300",
                  selectedUserId === user.id 
                    ? "border-blue-500/50 shadow-xl shadow-blue-500/5 ring-1 ring-blue-500/20" 
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    <div className="relative">
                      <div className={cn(
                        "w-12 h-12 rounded-2xl flex items-center justify-center text-sm font-bold uppercase transition-transform group-hover:scale-110 shadow-inner border",
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-zinc-900 group-hover:text-blue-600 transition-colors uppercase tracking-tight">{user.name}</h4>
        ...
        <AnimatePresence>
          {selectedUserId && (
            ...
            >
              <div className="bg-white/90 backdrop-blur-xl border border-zinc-200/60 rounded-2xl shadow-2xl p-6 flex flex-col h-full relative overflow-hidden">
                {/* Ambient Glow */}
                ...
                <div className="space-y-4 overflow-auto max-h-[500px] pr-2 custom-scrollbar relative z-10">
                        ...
                        <div className="flex items-center space-x-3">
                          <div className={cn(
                            ...
                            selectedUser?.assignedProjects.includes(project.id) 
                              ...
                              : "bg-zinc-100 text-zinc-400 shadow-inner"
                          )}>
                ...
                <div className="mt-auto pt-8 relative z-10">
                  <div className="bg-blue-50/30 rounded-xl p-4 border border-blue-100/50 mb-6 group/info overflow-hidden">
                    ...
                    <p className="text-[11px] text-blue-600/80 leading-relaxed font-medium">
                      正在修改租户成员的访问逻辑。该用户将继承 <strong className="text-blue-700">{selectedUser?.role}</strong> 角色的所有全局属性，并获准访问上述选定的项目核心资源。
                    </p>
                    <div className="absolute bottom-0 right-0 w-16 h-16 bg-blue-500/5 rotate-45 translate-x-8 translate-y-8" />
                  </div>
                  <Button className="w-full h-10 bg-zinc-900 hover:bg-blue-600 text-white text-xs font-bold rounded-xl shadow-lg transition-all group/btn overflow-hidden relative">
...
function MonitorCard({ label, value, icon: Icon, color }: { label: string; value: string | number; icon: any; color: string }) {
  const colorMap: any = {
    ...
    "text-amber-600": "text-amber-600 bg-amber-50",
