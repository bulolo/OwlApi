  ...
  return (
    <div className="max-w-3xl mx-auto py-10">
      <div className="mb-8 text-center">
        ...
        <h1 className="text-2xl font-bold text-foreground tracking-tight">部署新的网关节点</h1>
        <p className="text-sm text-muted-foreground mt-1 font-medium">创建节点后，使用生成的 Token 在目标服务器上启动 Gateway。</p>
      </div>

      <div className="flex items-center justify-center space-x-4 mb-10">
        <StepIndicator num={1} active={step >= 1} label="配置信息" />
        <div className={`w-12 h-[1px] ${step >= 2 ? 'bg-primary' : 'bg-zinc-200'}`} />
        <StepIndicator num={2} active={step >= 2} label="安装部署" />
      </div>

      <div className="bg-white border border-border-subtle rounded-lg shadow-card overflow-hidden">
        {step === 1 && (
          <div className="p-6 space-y-4">
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">仅用于在控制台中识别该节点，建议包含位置和用途。</p>
            </div>
            ...
            <div className="bg-primary/10 border border-primary/20 p-4 rounded-lg flex items-start gap-3">
              ...
              <div>
                <h4 className="text-xs font-bold text-primary uppercase">通用接入模式</h4>
            ...
            <div className="pt-4 flex justify-end">
              <Button onClick={handleCreate} className="h-9 px-4 text-xs font-bold shadow-sm">
                创建并生成 Token
        ...
        {step === 2 && created && (
          <div className="p-6 space-y-4">
            <div>
              <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-3">1. 创建 docker-compose.yml</p>
              <div className="bg-zinc-900 rounded-lg p-4 font-mono text-sm text-white relative group">
                <div className="absolute top-3 right-3">
                  <Button size="icon-sm" variant="ghost" className="text-muted-foreground hover:text-white hover:bg-zinc-800 rounded" onClick={handleCopy}>
            ...
            <div>
              <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-3">2. 启动网关</p>
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
        active ? 'bg-primary text-white shadow-sm' : 'bg-zinc-100 text-muted-foreground'
      }`}>
        {num}
      </div>
      <span className={`text-2xs font-bold uppercase tracking-wide ${active ? 'text-primary' : 'text-muted-foreground'}`}>
        {label}
      </span>
    </div>
  )
