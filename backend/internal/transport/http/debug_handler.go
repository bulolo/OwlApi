...
// @Param body body object{endpoint_id=int,env_id=int,params=object,ignore_scripts=bool} true "测试参数"
...
func (h *QueryTestHandler) HandleTestQuery(c *gin.Context) {
	...
	result, err := h.queries.Execute(c.Request.Context(), tenantID, envID, endpoint, req.Params)
	if err != nil {
		FailErr(c, err)
		return
	}
